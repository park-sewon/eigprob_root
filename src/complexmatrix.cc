...
//fprintf(stderr," +- Kreiere 1m: %x\n",this);
  if ( (rows == 0)||  (columns == 0 ) ) { 
     fprintf(stderr,"Error in dimensioning COMPLEX matrix of size [%d,%d]\n",
         rows,columns);
     exit(1);
     }
...
COMPLEXMATRIX operator + (const COMPLEXMATRIX& x, const COMPLEXMATRIX& y) {
  ...
  if ( x.maxrow != y.maxrow || x.maxcolumn != y.maxcolumn) { 
     fprintf(stderr,"Error in adding COMPLEX matrices of different sizes \n");
     exit(1);
     }
...
COMPLEXMATRIX operator * (const COMPLEXMATRIX& x ,const COMPLEXMATRIX& y) {
  ...
  if ( x.maxcolumn != y.maxrow) { 
     fprintf(stderr,"Error in multiplying COMPLEX matrices of different sizes \n");
     exit(1);
     }
...
COMPLEX&  COMPLEXMATRIX::element (unsigned int i, unsigned int j) const {
  if ((i>= maxrow) || (j>= maxcolumn) ) {
     fprintf(stderr,"Illegal indices [%d,%d] for COMPLEX matrix [%d,%d]\n",
...
void COMPLEXMATRIX::adderror (sizetype error)
{ 
...
void COMPLEXMATRIX::seterror (sizetype error)
{ 
  for (unsigned int i=0;i<(*this).maxcolumn;i++) 
  for (unsigned int j=0;j<(*this).maxrow;j++) 
...
void COMPLEXMATRIX::geterror (sizetype& error) const
{
  unsigned int i,j;
  sizetype lerror; 
